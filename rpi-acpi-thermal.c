...
static void __acpi_thermal_trips_update(struct acpi_thermal *tz, int flag)
{
	...
	for (i = 0; i < ACPI_THERMAL_MAX_ACTIVE; i++) {
		...
		if (flag == ACPI_TRIPS_INIT || ((flag & ACPI_TRIPS_ACTIVE) &&
			...
			status = acpi_evaluate_integer(tz->device->handle,
						       name, NULL, &tmp);
			dev_info(&tz->device->dev,"Device Handle: %p\n",
					tz->device->handle);
					  ...
					  bool bind)
{
	...
	for (i = 0; i < ACPI_THERMAL_MAX_ACTIVE; i++) {
		...
		for (j = 0; j < tz->trips.active[i].devices.count; j++) {
			...
			pr_info("Cooling device (active): handle=%p, bind=%d, dev=%p, status={present=%d, enabled=%d, functioning=%d}, name=%s, hid=%s\n",
				handle, bind, dev,
...
static int acpi_thermal_register_thermal_zone(struct acpi_thermal *tz)
{
	...
	tz->thermal_zone = thermal_zone_device_register_with_trips("rpitherm",
								   tz->trip_table,
								   trip_count,
								   0, tz,
